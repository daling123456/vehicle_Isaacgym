...
class MLPEncode(nn.Module):
    def __init__(self, device, model_cfg, activation_fn, output_activation_fn = None, small_init= False):
        ...
        self.base_obs_size=self.obs_dim+25   #obs+action?
        self.history_length=self.tsteps
        self.state_history_dim=self.base_obs_size * self.history_length
        # print(self.state_history_dim)
        self.historyencoder_type=model_cfg.actor.historyencoder_type
        ...
        if self.state_history_dim > 0:
            self.state_history_encoder = StateHistoryEncoder(activation_fn, self.tsteps, self.state_history_dim, history_latent_dim, self.historyencoder_type)
...
class Actor(nn.Module):
    def __init__(self, device, model_cfg, activation_fn, output_activation_fn = None, small_init= False):
        ...
        suspensin_hidden_shape = model_cfg.actor.suspension_hidden_shape  # suspension net
        steer_hidden_shape = model_cfg.actor.steer_hidden_shape  # steer_net
        prop_hidden_shape = model_cfg.actor.prop_hidden_shape   #特权向量编码器结构
        geom_hidden_shape = model_cfg.actor.geom_hidden_shape   #地形信息编码器结构

        #输入参数尺寸
        self.vision_dim = model_cfg.actor.vision_dim  # 视觉输入维度
        self.obs_dim = model_cfg.actor.obs_dim  # 本体感知观测空间大小
        self.prop_dim = model_cfg.actor.prop_dim    #特权信息大小
        self.geom_dim = model_cfg.actor.geom_dim

        #向量大小
        vision_latent_dim=model_cfg.actor.vision_latent_dim     #视觉向量大小
        prop_latent_dim=model_cfg.actor.prop_latent_dim     #特权信息向量大小
        geom_latent_dim=model_cfg.actor.geom_latent_dim     #
        history_latent_dim=model_cfg.actor.history_latent_dim   #状态历史编码向量大小
        self.suspension_latten_dim=model_cfg.actor.suspension_latten_dim     #主动悬架模块输入参数大小
        ...
        steer_input_size=self.steer_latten_dim
        steer_output_dim = model_cfg.actor.steer_output_dim  # 13

        # self.base_obs_size=self.obs_dim+1   #obs+action?
        # self.history_length=self.tsteps
        # self.state_history_dim=self.base_obs_size * self.history_length
        # self.historyencoder_type=model_cfg.actor.historyencoder_type
        ...
        #     # raise IOError("Not implemented self.geom_dim")
        #     print("Not implemented self.geom_dim")
        self.action_mlp = MLP(device, actor_hidden_shape, self.actor_input_size, self.actor_output_size,
                              self.activation_fn, self.output_activation_fn, self.small_init)
        self.suspension_mlp = MLP(device, suspensin_hidden_shape, suspension_input_size, suspension_output_size,
                                  self.activation_fn, self.output_activation_fn, self.small_init)
        self.steer_mlp = MLP(device, steer_hidden_shape, steer_input_size, steer_output_dim, self.activation_fn,
                             self.output_activation_fn, self.small_init)
        self.prop_encoder= MLP(device, prop_hidden_shape, self.prop_dim, prop_latent_dim, self.activation_fn,
                             self.output_activation_fn, self.small_init)
        if self.geom_dim > 0:
            self.geom_encoder = MLP(device, geom_hidden_shape, self.geom_dim, geom_latent_dim, self.activation_fn,
                                    self.output_activation_fn, self.small_init)
