...
from vehicle_Isaacgym.vehicle.utils.terrain import perlin
...
class TMBoxTerrain(Terrain):
    ...
    def prepare(self):
        ...
        self.terrain_cell_bounds = {
            "x_min": torch.zeros(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols, device=self.env.device),
            "y_min": torch.zeros(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols, device=self.env.device),
            "x_max": torch.zeros(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols, device=self.env.device),
            "y_max": torch.zeros(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols, device=self.env.device)
        }
        min_friction, max_friction = self.env.cfg.domain_rand.ground_friction_range
        self.terrain_cell_frictions = torch.rand(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols,
        ...
        self.terrain_cell_restitutions = torch.rand(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols,
                                                 dtype=torch.float, device=self.env.device,
                                                 requires_grad=False) * (
                                              max_restitution - min_restitution) + min_restitution

        min_roughness, max_roughness = self.env.cfg.domain_rand.tile_roughness_range
        self.terrain_cell_roughnesses = torch.rand(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols,
                                                 dtype=torch.float, device=self.env.device,
                                                 requires_grad=False) * (
                                              max_roughness - min_roughness) + min_roughness

        lengthwise_density = int(self.env.cfg.env.terrain.terrain_length / self.env.cfg.env.terrain.horizontal_scale)
        widthwise_density = int(self.env.cfg.env.terrain.terrain_width / self.env.cfg.env.terrain.horizontal_scale)

        self.stair_heights_samples = torch.zeros(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols,
                                                 dtype=torch.float, device=self.env.device,
                                                 requires_grad=False).repeat_interleave(
                                                 lengthwise_density, dim=0).repeat_interleave(widthwise_density, dim=1)

        self.stair_runs_samples = torch.zeros(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols,
                                                 dtype=torch.float, device=self.env.device,
                                                 requires_grad=False).repeat_interleave(
                                                 lengthwise_density, dim=0).repeat_interleave(widthwise_density, dim=1)

        self.stair_oris_samples = torch.zeros(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols,
                                                 dtype=torch.float, device=self.env.device,
                                                 requires_grad=False).repeat_interleave(
                                                 lengthwise_density, dim=0).repeat_interleave(widthwise_density, dim=1)

        self.env.min_height, self.env.max_height = 0, 0
        self.terrain_cell_heights = torch.rand(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols,
                                               dtype=torch.float, device=self.env.device,
                                               requires_grad=False) * (
                                            self.env.max_height - self.env.min_height) + self.env.min_height

        self.terrain_cell_center_heights = torch.zeros(self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols, dtype=torch.float, device=self.env.device, requires_grad=False)

        lengthwise_density = int(self.env.cfg.env.terrain.terrain_length / self.env.cfg.env.terrain.horizontal_scale)
        widthwise_density = int(self.env.cfg.env.terrain.terrain_width / self.env.cfg.env.terrain.horizontal_scale)
        self.height_samples = torch.tensor(self.terrain_cell_heights).view(self.env.cfg.env.terrain.num_rows,
            self.env.cfg.env.terrain.num_cols).repeat_interleave(lengthwise_density, dim=0).repeat_interleave(widthwise_density, dim=1).to(
            self.env.device) / self.env.cfg.env.terrain.vertical_scale

        n_border_boxes = self.env.cfg.env.terrain.num_border_boxes

        for i in range(self.env.cfg.env.terrain.num_rows):
            for j in range(self.env.cfg.env.terrain.num_cols):
                # border_px = int(self.env.cfg.env.terrain.border_size / self.env.cfg.env.terrain.horizontal_scale)
                ...
                horizontal_scale = self.env.cfg.env.terrain.horizontal_scale  # / self.env.cfg.env.terrain.num_rows
                vertical_scale = self.env.cfg.env.terrain.vertical_scale  # / self.env.cfg.env.terrain.num_cols

                tm_params = gymapi.TriangleMeshParams()
                vertices, triangles = terrain_utils.convert_heightfield_to_trimesh(heightfield_segment_raw,
                                                                                   horizontal_scale,
                                                                                   vertical_scale,
                                                                                   self.env.cfg.env.terrain.slope_treshold)
                ...
                if self.env.terrain_props is not None:
                    if self.env.cfg.env.terrain.curriculum:
                        terrain_type = self.env.terrain_props["type"][i, j]
                        difficulty = self.env.cfg.env.terrain.difficulty_scale
                    else:
                        terrain_type = self.env.terrain_props["type"][i, j]
                        difficulty = self.env.cfg.env.terrain.difficulty_scale
                    if i < n_border_boxes or i >= self.env.cfg.env.terrain.num_rows - n_border_boxes or j < n_border_boxes or j >= self.env.cfg.env.terrain.num_cols - n_border_boxes:
                        terrain_type = 0
                        self.terrain_cell_roughnesses[i, j] = 0.0
                        self.terrain_cell_frictions[i, j] = 3.0
                else:
                    if self.env.cfg.env.terrain.curriculum:
                        terrain_type = (j - n_border_boxes) / (self.env.cfg.env.terrain.num_cols - 2 * n_border_boxes) + 0.001
                        difficulty = (i - n_border_boxes) / (self.env.cfg.env.terrain.num_rows - 2 * n_border_boxes) * self.env.cfg.env.terrain.difficulty_scale
                    else:
                        terrain_type = np.random.random()
                        difficulty = self.env.cfg.env.terrain.difficulty_scale * np.random.random()
                    if i < n_border_boxes or i >= self.env.cfg.env.terrain.num_rows - n_border_boxes or j < n_border_boxes or j >= self.env.cfg.env.terrain.num_cols - n_border_boxes:
                        terrain_type = 0
                        self.terrain_cell_roughnesses[i, j] = 0.0
                        self.terrain_cell_frictions[i, j] = 3.0
                cume_props = np.cumsum(self.env.cfg.env.terrain.terrain_props)
                ...
                if self.env.custom_heightmap is not None:
                    ...
                    self.height_samples[start_px:end_px, start_py:end_py] = self.env.custom_heightmap[start_px:end_px, start_py:end_py] / self.env.cfg.env.terrain.vertical_scale
                else:
                    if terrain_type <= cume_props[0]: # flat/rough
                        ...
                        if self.env.terrain_props is not None:
                            ...
                            step_run = (self.env.cfg.env.terrain.max_step_run - self.env.cfg.env.terrain.min_step_run) * torch.rand(1) + self.env.cfg.env.terrain.min_step_run
                            step_height = self.env.cfg.env.terrain.max_step_height * difficulty
                        num_steps = 8
                        for k in range(num_steps):
                            step_px =  int(step_run * k / self.env.cfg.env.terrain.horizontal_scale)
                            if self.env.terrain.cfg.length_per_env_pixels - 1.0 / self.env.cfg.env.terrain.horizontal_scale <= step_px * 2:
                                k -= 1
                                break
                            height = k * step_height
                            self.height_samples[start_px+step_px:end_px-step_px, start_py+step_px:end_py-step_px] = height / self.env.cfg.env.terrain.vertical_scale
                            if k < num_steps - 1:
                        ...
                        max_height = int(slope * (self.env.cfg.env.terrain.horizontal_scale / self.env.cfg.env.terrain.vertical_scale) * (width / 2))
                        self.height_samples[start_px:end_px, start_py:end_py] = torch.tensor((max_height * xx * yy))

                        platform_size = 1.5

                        platform_size = int(platform_size / self.env.cfg.env.terrain.horizontal_scale / 2)
                        ...
                        self.terrain_cell_center_heights[i, j] = max_h * self.env.cfg.env.terrain.vertical_scale
                    elif terrain_type <= cume_props[4]:  # pit
                        self.height_samples[start_px:end_px, start_py:end_py] = -5.0 / self.env.cfg.env.terrain.vertical_scale
                        self.terrain_cell_center_heights[i, j] = 0
                ...
                lin_x = np.linspace(0, self.env.cfg.env.terrain.terrain_length * 4, self.env.terrain.cfg.width_per_env_pixels,
                                    endpoint=False)
                lin_y = np.linspace(0, self.env.cfg.env.terrain.terrain_width * 4, self.env.terrain.cfg.length_per_env_pixels,
                                    endpoint=False)
                x, y = np.meshgrid(lin_x, lin_y)
                perlin_seed = i * self.env.cfg.env.terrain.num_cols + j
                self.height_samples[start_px:end_px, start_py:end_py] = self.height_samples[start_px:end_px, start_py:end_py] + torch.tensor(perlin(x, y, seed=perlin_seed), device=self.env.device) * float(roughness) / self.env.cfg.env.terrain.vertical_scale

                
        self.friction_samples = torch.tensor(self.terrain_cell_frictions).view(self.env.cfg.env.terrain.num_rows,
                                                                               self.env.cfg.env.terrain.num_cols).repeat_interleave(
            lengthwise_density, dim=0).repeat_interleave(widthwise_density, dim=1).to(self.env.device)
        self.roughness_samples = torch.tensor(self.terrain_cell_roughnesses).view(self.env.cfg.env.terrain.num_rows,
                                                                               self.env.cfg.env.terrain.num_cols).repeat_interleave(
            lengthwise_density, dim=0).repeat_interleave(widthwise_density, dim=1).to(self.env.device)

        self.height_samples[:10, :] = 3.0 / self.env.cfg.env.terrain.vertical_scale
        self.height_samples[-10:, :] = 3.0 / self.env.cfg.env.terrain.vertical_scale
        self.height_samples[:, :10] = 3.0 / self.env.cfg.env.terrain.vertical_scale
        self.height_samples[:, -10:] = 3.0 / self.env.cfg.env.terrain.vertical_scale
    ...
    def initialize(self):
        ...
        self.env.terrain_actors = np.full((self.env.cfg.env.terrain.num_rows, self.env.cfg.env.terrain.num_cols), None)
        ...
        for i in range(self.env.cfg.env.terrain.num_rows):
            for j in range(self.env.cfg.env.terrain.num_cols):
                ...
                # lin_x = np.linspace(0, self.env.cfg.env.terrain.terrain_length * 3, heightfield_segment_raw.shape[1], endpoint=False)
                # lin_y = np.linspace(0, self.env.cfg.env.terrain.terrain_width * 3, heightfield_segment_raw.shape[0], endpoint=False)
                # x, y = np.meshgrid(lin_x, lin_y)
                # heightfield_segment_raw = heightfield_segment_raw + perlin(x, y, seed=2) * float(roughness) / self.env.cfg.env.terrain.vertical_scale

                tm_params = gymapi.TriangleMeshParams()
                # print(heightfield_segment_raw.shape)
                vertices, triangles = terrain_utils.convert_heightfield_to_trimesh(heightfield_segment_raw,
                                                                                   horizontal_scale,
                                                                                   vertical_scale,
                                                                                   self.env.cfg.env.terrain.slope_treshold)
        ...
        self.roughness_samples = torch.tensor(self.terrain_cell_roughnesses).view(self.env.cfg.env.terrain.num_rows,
                                                                                  self.env.cfg.env.terrain.num_cols).repeat_interleave(
            lengthwise_density, dim=0).repeat_interleave(widthwise_density, dim=1).to(self.env.device)
        self.restitution_samples = torch.tensor(self.terrain_cell_restitutions).view(self.env.cfg.env.terrain.num_rows,
                                                                                     self.env.cfg.env.terrain.num_cols).repeat_interleave(
            lengthwise_density, dim=0).repeat_interleave(widthwise_density, dim=1).to(self.env.device)
